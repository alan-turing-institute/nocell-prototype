...
;; Check conversions nocell -> cell -> grid -> sxml -> fods
;;
...
(define check:sxml->fods   (check-convert srl:sxml->xml))
...
(module+ test
  (for ([test-dir (directory-list "test-examples" #:build? #t)])
    ...
    (match-let
        ([(list nocell-path cell-path grid-path sxml-path fods-path)
          (map paths/suffix '(".nocell" ".cell" ".grid" ".sxml" ".fods.rkt"))])
      (check:nocell->cell nocell-path cell-path)
      (check:cell->grid   cell-path   grid-path)
      (check:grid->sxml   grid-path   sxml-path)
      (check:sxml->fods   sxml-path   fods-path))))
